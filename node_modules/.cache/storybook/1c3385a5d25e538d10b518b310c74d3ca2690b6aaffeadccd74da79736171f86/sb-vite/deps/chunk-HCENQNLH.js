...
function useAutocomplete(props) {
  ...
  const ignoreFocus = React86.useRef(false);
  const firstFocus = React86.useRef(true);
  const inputRef = React86.useRef(null);
  const listboxRef = React86.useRef(null);
  const [anchorEl, setAnchorEl] = React86.useState(null);
  const [focusedTag, setFocusedTag] = React86.useState(-1);
  const defaultHighlighted = autoHighlight ? 0 : -1;
  const highlightedIndexRef = React86.useRef(defaultHighlighted);
  ...
  const resetInputValue = React86.useCallback((event, newValue) => {
  ...
  const [inputPristine, setInputPristine] = React86.useState(true);
  ...
  const syncHighlightedIndex = React86.useCallback(() => {
    if (!popupOpen) {
      return;
    }
  ...
  const isTouch = React86.useRef(false);
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-HCENQNLH.js.map
